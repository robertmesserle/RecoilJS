...
__cleanUpDirectories = function(folders) {
  var directories, folderConfig, onlyDirectories, removeDirectoryIfExists, toFolderPaths, _i, _len, _results;

  toFolderPaths = function(f) {
    return path.join(folderConfig.folder, f);
  };
  onlyDirectories = function(f) {
    return fs.statSync(f).isDirectory();
  };
  removeDirectoryIfExists = function(dir) {
    var err;

    if (fs.existsSync(dir)) {
      try {
        fs.rmdirSync(dir);
        return logger.success("Deleted empty directory [[ " + dir + " ]]");
      } catch (_error) {
        err = _error;
  ...
  for (_i = 0, _len = folders.length; _i < _len; _i++) {
    ...
    directories = wrench.readdirSyncRecursive(folderConfig.folder).map(toFolderPaths).filter(onlyDirectories).push(folderConfig.folder);
    _results.push(_.sortBy(directories, 'length').reverse().forEach(removeDirectoryIfExists));
  }
  return _results;
};
